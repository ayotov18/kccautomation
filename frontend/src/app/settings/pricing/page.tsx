...
export default function PricingDefaultsPage() {
  ...
  useEffect(() => {
    document.title = 'Ценови настройки · KCC';
    api
      .getPricingDefaults()
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-7">
        <Breadcrumbs
          items={[
            ...
            { label: 'Ценови настройки' },
          ]}
        />

        <header className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">Ценови настройки</h1>
            <p className="text-sm text-content-tertiary mt-1 max-w-xl">
              Стойностите се инжектират в AI заявката и се използват като
              позиции по подразбиране във всяка генерирана КСС. Промените се
              прилагат при следващата генерация.
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={save} disabled={saving === 'saving'} className="oe-btn-primary">
              {saving === 'saved' ? (
                <>
        ...
        {loading ? (
          ...
          <>
            {/* Presets */}
            <Section title="Профил" subtitle="Един клик за стандартна конфигурация">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className={`text-sm font-medium ${active ? 'text-sky-300' : 'text-content-primary'}`}>
                          {p.label}
                        </span>
            ...
            {/* Надбавки */}
            <Section title="Надбавки" subtitle="Прилагат се след преките разходи + ДР">
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <RateRow label="Зидар" value={state.labor_rates.mason} onChange={(b) => setRate('mason', b)} />
                <RateRow label="Кофражист" value={state.labor_rates.formwork} onChange={(b) => setRate('formwork', b)} />
                <RateRow label="Армировач" value={state.labor_rates.rebar} onChange={(b) => setRate('rebar', b)} />
                <RateRow label="Бояджия" value={state.labor_rates.painter} onChange={(b) => setRate('painter', b)} />
                <RateRow label="Електротехник" value={state.labor_rates.electrician} onChange={(b) => setRate('electrician', b)} />
                <RateRow label="Водопроводчик" value={state.labor_rates.plumber} onChange={(b) => setRate('plumber', b)} />
                <RateRow label="Заварчик" value={state.labor_rates.welder} onChange={(b) => setRate('welder', b)} />
                <RateRow label="Помощник" value={state.labor_rates.helper} onChange={(b) => setRate('helper', b)} />
              </div>
            </Section>
          </>
        )}
  ...
  function setRate(key: keyof PricingDefaults['labor_rates'], band: LaborRateBand) {
...
function Section({
  ...
  return (
    <section className="oe-card p-6 space-y-4">
      <div>
